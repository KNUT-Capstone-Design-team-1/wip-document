...
flowchart TD
    ...
    is_exist_pill_data_table -->|아니오| reqeust_pill_data_table_schema["PillData 테이블 스키마 조회 (pill-data-table-schema)"]
    ...
    check_require_update -->|예| reqeust_pill_data_table_schema
    check_require_update -->|아니오| search((식별 검색 혹은 이미지 검색 수행))


    reqeust_pill_data_table_schema --> drop_pill_data_table[PillData 테이블 DROP]
    drop_pill_data_table --> create_pill_data_table[PillData 테이블 CREATE]
    create_pill_data_table --> request_pill_data_resource["PillData 요청 (pill-data-resource)"]
    request_pill_data_resource --> insert_pill_data[PillData 테이블 INSERT]
